...
-(BOOL) n_txtbox_open:(NSString*)nsstring
{
	...
	BOOL ret = n_txt_load_utf8( txtbox->txt_data, str );
	if ( ret ) { n_string_free( str ); return FALSE; }


	if (
		( txtbox->txt_data->unicode == N_TXT_UNICODE_NIL )
		&&
		( txtbox->txt_data->newline != N_TXT_NEWLINE_BINARY )
	)
	{
	...
	n_txt_decoration_init( txtbox->txt_data, txtbox->txt_deco );

//NSLog( @"Unicode %d : Newline %d", n_txt->unicode, n_txt->newline );

	txtbox->path = nsstring;
...
//NSLog( @"draggingEntered" );
	...
	if ( self.txtbox->mode == N_MAC_TXTBOX_MODE_LISTBOX ) { return NSDragOperationNone; }
	if ( self.txtbox->mode == N_MAC_TXTBOX_MODE_FINDBOX ) { return NSDragOperationNone; }
	if ( self.txtbox->mode == N_MAC_TXTBOX_MODE_ONELINE ) { return NSDragOperationNone; }
